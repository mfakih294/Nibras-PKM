$.continuousCalendar = {};$.continuousCalendar.version = '1.2.3';$.continuousCalendar.released = '2012-06-04'
/*
...
DateRange.prototype = {
  ...
  isPermittedRange: function(minimumSize, disableWeekends, outerRange) {
      return this.hasValidSize(minimumSize)
...
(function($) {
  $.fn.continuousCalendar = function(options) {
    ...
    function _continuousCalendar(options) {
      ...
      function dateStyles(date) { return $.trim(['date', backgroundBy(date), disabledOrNot(date)].sort().join(' ')) }
        // , todayStyle(date), holidayStyle(date)
...
//          console.log(DateFormat.format(date.getOnlyDate(), 'n/j/Y', params.locale))

        var disabledDay = params.disabledDatesMy.indexOf(DateFormat.format(date.getOnlyDate(), 'n/j/Y', params.locale))
...
//          console.log(params.disabledDatesMy)
        return disabledDay != -1 ? 'disabled' : ''
      }
      ...
      function mouseDown(event) {
        ...
        function enabledCell(elem) { return isDateCell(elem)
//                && isEnabled(elem)
        }
