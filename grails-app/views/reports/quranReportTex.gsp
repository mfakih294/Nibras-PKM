...
<g:each in="${mcs.Writing.executeQuery('from Writing i where i.type.code = ?  order by i.orderInBook', ['surah'])}" var="w"><g:set var="count" value="${1}"/>
    \section{${w.orderInBook + ' سورة ' + w.summary}} [${count++}] <g:each in="${app.IndexCard.executeQuery('from IndexCard i where i.recordId = ? and i.type.code = ? order by i.orderInWriting', [w.id.toString(), 'aya'])}"  var="r">  <g:if test="${r.isNewSection}">\normalsize \newline [${count++}] </g:if>
    ${r.shortDescription} \small{(${r.orderInWriting})} \normalsize </g:each> \normalsize</g:each>
